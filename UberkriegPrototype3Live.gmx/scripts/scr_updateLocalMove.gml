        ...
        and  global.rangeCheck[newpos_x,newpos_y].move_cost = -1 ){
            if( //total move points needed to move into new tile
                ...
                <= other.max_move_points){
                    ...
                    global.rangeCheck[newpos_x,newpos_y].move_cost = move_cost + scr_check_move_cost(other.move_check_target,obj_map.terrains[newpos_x,newpos_y]);
...
//switch blue tiles to clear if checking enemy units
    
if (global.P_Turn.team != move_check_target.team ){
    with (obj_checker_tile) if (sprite_index != -1) sprite_index = spr_rangecheck_clear; 
    }
else if (move_check_target.isCommander ) {
    with (obj_checker_tile) if (sprite_index != -1) sprite_index = spr_rangecheck_clear;     
    }
